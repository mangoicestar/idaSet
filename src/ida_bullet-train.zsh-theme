...
#  BULLETTRAIN_DIR_BG=blank
fi
if [ ! -n "${BULLETTRAIN_DIR_FG+1}" ]; then
BULLETTRAIN_DIR_FG=%{$fg_bold[blue]%
#BULLETTRAIN_DIR_FG=magenta
#BULLETTRAIN_DIR_FG=white
#BULLETTRAIN_DIR_FG=%{$fg[green]%}
...
#BULLETTRAIN_GIT_FG=black
  BULLETTRAIN_GIT_FG=%{$fg_bold[green]%
fi
...
#prompt_segment $BULLETTRAIN_VIRTUALENV_BG $BULLETTRAIN_VIRTUALENV_FG $BULLETTRAIN_VIRTUALENV_PREFIX" $(basename $virtualenv_path)"
    prompt_segment $BULLETTRAIN_VIRTUALENV_PREFIX" $(basename $virtualenv_path)"
...
#    prompt_segment $BULLETTRAIN_VIRTUALENV_BG $BULLETTRAIN_VIRTUALENV_FG $BULLETTRAIN_VIRTUALENV_PREFIX" $(pyenv version | sed -e 's/ (set.*$//' | tr '\n' ' ' | sed 's/.$//')"
    prompt_segment $BULLETTRAIN_VIRTUALENV_PREFIX" $(pyenv version | sed -e 's/ (set.*$//' | tr '\n' ' ' | sed 's/.$//')"
  fi
}
...
#prompt_segment $BULLETTRAIN_NVM_PREFIX$nvm_prompt
  prompt_segment
}
